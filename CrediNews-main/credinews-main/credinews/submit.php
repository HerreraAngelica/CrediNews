...
if ($userData['submission_count'] >= $submissionLimit) {
    // Check if 24 hours have passed since the first submission
    ...
    if ($timeDiff < 86400) { // 24 hours in seconds
        ...
        // Reset submission count if 24 hours have passed
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $canSubmit) {
    ...
    // If no errors, insert article
    if (empty($errors)) {
        // Encrypt content if needed (for sensitive articles)
